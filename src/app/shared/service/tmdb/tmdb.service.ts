...
export class TmdbService {
  ...
  private URL_MODEL_TRENDING = 'http://localhost:8080/trending';
  private URL_MODEL_RECOMMEND = 'http://localhost:8080/recommend/';
  private URL_MODEL_USERRATING = 'http://localhost:8080/also-like/50/';
  ...
  getTrending() : Observable<any[]> {
    return this.http.get<any[]>(`${this.URL_MODEL_TRENDING}`);
  }

  getRecommended(name:String) : Observable<any[]> {
    return this.http.get<any[]>(`${this.URL_MODEL_RECOMMEND}${name}`);
  }

  getPredictions(name:String) : Observable<any[]> {
    return this.http.get<any[]>(`${this.URL_MODEL_USERRATING}${name}`);
  }
